...
static int Audio_i2s0_SideGen_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
    ...
    if (mi2s0_sidegen_control)
    {
        ...
        Afe_Set_Reg(AUDIO_TOP_CON1, 0x1 << 7,  0x1 << 7); /* lenovo-sw zhouwl, 2014-12-30,from mtk,i2s3 clock gated */
        if (mi2s0_extcodec_echoref_control == true)
        {
            ...
            SetCLkMclk(Soc_Aud_I2S3, samplerate); /* lenovo-sw zhouwl, 2014-12-30,from mtk,i2s3 mclk */
            SetSampleRate(Soc_Aud_Digital_Block_MEM_I2S,  samplerate);

            Audio_I2S_Dac |= (Soc_Aud_LR_SWAP_NO_SWAP << 31);
            /* lenovo-sw zhouwl, 2014-12-29,from mtk,i2sin from io_mux */
            Audio_I2S_Dac |= (1 << 28);
            /* lenovo-sw zhouwl, 2014-12-29,from mtk,i2sin from io_mux */
        ...
        if (GetMemoryPathEnable(Soc_Aud_Digital_Block_I2S_OUT_2) == false)
        {
            ...
            Afe_Set_Reg(AUDIO_TOP_CON1, 0x0 << 7,  0x1 << 7); /* lenovo-sw zhouwl, 2014-12-30,from mtk,i2s3 not clock gated */
