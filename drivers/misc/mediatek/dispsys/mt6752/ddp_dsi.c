...
void DSI_PHY_TIMCONFIG(DISP_MODULE_ENUM module, cmdqRecHandle cmdq, LCM_DSI_PARAMS *dsi_params)
{
	...
	// +3 is recommended from designer becauase of HW latency
	timcon0.HS_TRAIL = ((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 0x0a-8;
	#endif
...
int ddp_dsi_build_cmdq(DISP_MODULE_ENUM module, void *cmdq_trigger_handle, CMDQ_STATE state)
{
	...
	DSI_T0_INS t0;
//Lenovo-sw jixu modify for support esd read longpackage begin
	DSI_T0_INS t1;
...
//Lenovo-sw jixu modify for support esd read longpackage end
...
//Lenovo-sw jixu modify for support esd read longpackage end
	        ...
	        {
			//Lenovo-sw jixu modify for support esd read longpackage begin
		        ret = cmdqRecPoll(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[dsi_i])+0xc), 0x1, 0x00000001);
		        ret = cmdqRecWrite(cmdq_trigger_handle, disp_addr_convert((unsigned long)(DSI_REG[dsi_i])+0xc), 0x0, 0x00000001);
			//Lenovo-sw jixu modify for support esd read longpackage end
	        }
#if 0
			else // DSI1
...
//Lenovo-sw jixu modify for support esd read longpackage begin
			...
			if(packet_type == 0x1A || packet_type == 0x1C)
			{
				...
				DISPCHECK("DSI read long packet size: %d\n", recv_data_cnt);
				{
						...
						pData = (unsigned char *)&read_data1;
						ret = 0;
						for(ii=0;ii<dsi_params->lcm_esd_check_table[i].count;ii++)
						{
							DISPCHECK("%d:read_data = 0x%d,cmp_data = 0x%x\n",ii,*pData,dsi_params->lcm_esd_check_table[i].para_list[ii]);
							if(*pData != dsi_params->lcm_esd_check_table[i].para_list[ii])
							{
								ret = 1;
								i = 3;
								break;
							}
							if(ii == 3) pData = &read_data2;
							else if(ii == 7) pData = &read_data3;
							else if(ii>11) break;
							else pData++;
						}
				}
